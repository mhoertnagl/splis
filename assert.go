...
func (a *assertion) AssertLen(seq []Node, l int, funName string) bool {
	if a.ok && len(seq) != l {
		msg := fmt.Sprintf("%s requires exactly [%d] arguments.\n", funName, l)
		a.ok = false
		a.err = NewErrNode(msg)
	}
	return a.Failed()
...
func (a *assertion) AssertMin(seq []Node, min int, funName string) bool {
	if a.ok && len(seq) < min {
		msg := fmt.Sprintf("%s requires at least [%d] arguments.\n", funName, min)
		a.ok = false
		a.err = NewErrNode(msg)
	}
	return a.Failed()
...
func (a *assertion) AssertType(n Node, t NodeType, argName string) bool {
	if a.ok && n.Type() != t {
		msg := fmt.Sprintf("%s must be of type [%v] but is [%v].\n", argName, t.Name(), n.Type().Name())
		a.ok = false
		a.err = NewErrNode(msg)
	}
	return a.Failed()
